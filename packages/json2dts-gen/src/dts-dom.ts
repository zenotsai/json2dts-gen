...
export interface ArrayDeclaration extends DeclarationBase {
    kind: "array";
    name: string;
    type: Type;
}
...
export interface TypeTypeReference {
    kind: "type";
    type: Type;
}
...
export type TopLevelDeclaration =   TypeDeclaration | NamespaceMember | ExportEqualsDeclaration | ExportDefaultDeclaration | ExportNameDeclaration | ModuleDeclaration | EnumDeclaration | Import;
...
export const create = {
    ...
    arrayDec(name: string, type: Type): ArrayDeclaration {
        return {
            kind: "array",
            type, name
        };
    },
    type(name: string, type: Type): TypeDeclaration {
        return {
            kind: "type",
            type, name
        };
...
export function emit(rootDecl: TopLevelDeclaration, { rootFlags = ContextFlags.None, tripleSlashDirectives = [], singleLineJsDocComments = false }: EmitOptions = {}): string {
    ...
    function writeType(d: TypeDeclaration) {
        startWithDeclareOrExport(`type `);
        print(d.name);
        print(' = ');
